...
|---|---|---|---|---|
| `CRED_META_CLOUD` | Token de acesso da Meta WhatsApp Cloud API (permanente, de um System User) + Phone Number ID | Meta for Developers → seu App → WhatsApp → API Setup / System Users (ver passo 1 do GO-LIVE) | n8n → Credentials → New → "WhatsApp API" (ou "Header Auth" apontando para o Graph API, conforme o node) → nome exato `CRED_META_CLOUD` | W1, W2, W3, W4, W6 |
| `CRED_SUPABASE_ROTAVIVA` | `service_role` key do projeto Supabase Morgana Ops (schema `rotaviva`) | Supabase Dashboard → projeto Morgana Ops → Project Settings → API → `service_role` key (secreta) | n8n → Credentials → New → "HTTP Custom Auth" com headers `apikey: <service_role_key>` e `Authorization: Bearer <service_role_key>` → nome exato `CRED_SUPABASE_ROTAVIVA` | W1, W2, W3, W4, W5, W6 |
...
| `CRED_MS_EXCEL` | OAuth2 da conta Microsoft (OneDrive da Morgana Ops) com acesso à pasta de trabalho do espelho | Azure Portal → App registrations → novo app com permissões `Files.ReadWrite` e `offline_access` | n8n → Credentials → New → "Microsoft Excel OAuth2 API" → nome exato `CRED_MS_EXCEL` | W5 |
